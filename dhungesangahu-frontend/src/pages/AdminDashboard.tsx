...
export const AdminDashboard: React.FC = () => {
  ...
  // ADMISSION STEPS HANDLERS
  const handleCreateAdmissionStepSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAdmissionStep.num.trim() || !newAdmissionStep.title.trim()) {
      alert('Step Number and Title are required.');
      return;
    }

    const payload = {
      num: newAdmissionStep.num.trim(),
      title: newAdmissionStep.title.trim(),
      desc: newAdmissionStep.desc.trim(),
      icon: newAdmissionStep.icon,
      order: newAdmissionStep.order,
      details: newAdmissionStep.detailsText.split('\n').map(d => d.trim()).filter(Boolean)
    };

    try {
      setIsSubmitting(true);
      if (editingAdmissionStepId) {
        const updated = await updateAdmissionStep(editingAdmissionStepId, payload);
        setAdmissionSteps(admissionSteps.map(item => item.id === editingAdmissionStepId ? updated : item).sort((a, b) => a.order - b.order));
        triggerSuccess('Admission journey step updated successfully.');
      } else {
        const created = await createAdmissionStep(payload);
        setAdmissionSteps([...admissionSteps, created].sort((a, b) => a.order - b.order));
        triggerSuccess('Admission journey step created successfully.');
      }
      setShowAdmissionStepModal(false);
      setEditingAdmissionStepId(null);
      setNewAdmissionStep({ num: '', title: '', desc: '', icon: 'ClipboardList', detailsText: '', order: 0 });
    } catch (err: any) {
      console.error(err);
      alert('Error saving admission journey step.');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleEditAdmissionStep = (step: AdmissionStep) => {
    setEditingAdmissionStepId(step.id);
    setNewAdmissionStep({
      num: step.num,
      title: step.title,
      desc: step.desc,
      icon: step.icon || 'ClipboardList',
      detailsText: (step.details || []).join('\n'),
      order: step.order
    });
    setShowAdmissionStepModal(true);
  };

  const handleDeleteAdmissionStep = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this admission step?')) return;
    try {
      await deleteAdmissionStep(id);
      setAdmissionSteps(admissionSteps.filter(item => item.id !== id));
      triggerSuccess('Admission step deleted successfully.');
    } catch (err) {
      console.error(err);
      alert('Error deleting admission step.');
    }
  };
  ...
  const handleCreateOfficialSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newOfficial.name.trim() || !newOfficial.position.trim()) {
    ...
    const payload = {
      ...
      position: newOfficial.position.trim(),
      category: newOfficial.category,
      image: newOfficial.image.trim() || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=400',
      email: newOfficial.email.trim(),
      qualification: newOfficial.qualification.trim(),
      experience: newOfficial.experience.trim(),
      order: newOfficial.order
    };

    try {
      setIsSubmitting(true);
      if (editingOfficialId) {
        const updated = await updateOfficial(editingOfficialId, payload);
        setOfficials(officials.map(item => item.id === editingOfficialId ? updated : item).sort((a, b) => a.order - b.order));
        triggerSuccess('School official updated successfully.');
      } else {
        const created = await createOfficial(payload);
        setOfficials([...officials, created].sort((a, b) => a.order - b.order));
        triggerSuccess('School official created successfully.');
      }
      ...
      console.error(err);
      alert('Error saving school official.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteOfficial = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this school official?')) return;
    try {
      await deleteOfficial(id);
      setOfficials(officials.filter(item => item.id !== id));
      triggerSuccess('School official deleted successfully.');
    } catch (err) {
      console.error(err);
      alert('Error deleting school official.');
    }
  };
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-7 scrollbar-thin">
          <div>
            ...
            <nav className="space-y-1">
              ...
              <button
                onClick={() => { setActiveTab('admissionSteps'); setIsMobileSidebarOpen(false); }}
                className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-bold text-xs sm:text-sm transition-all cursor-pointer text-left ${
                  activeTab === 'admissionSteps'
                    ? 'bg-[#652d90] text-white shadow-md shadow-purple-900/30'
                    : 'hover:bg-slate-800 hover:text-white text-slate-400'
                }`}
              >
              ...
              <button
                onClick={() => { setActiveTab('officials'); setIsMobileSidebarOpen(false); }}
                className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-bold text-xs sm:text-sm transition-all cursor-pointer text-left ${
                  activeTab === 'officials'
                    ? 'bg-[#652d90] text-white shadow-md shadow-purple-900/30'
                    : 'hover:bg-slate-800 hover:text-white text-slate-400'
          ...
          <div>
            ...
            <div className="space-y-1.5 text-left">
              {[
                { name: 'School Team & Officials', icon: Users, status: 'Active' },
          ...
          {loading ? (
              ...
              {activeTab === 'overview' && (
                <div className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-12 gap-4">
                    ...
                    {/* Card 11 */}
                    <div 
                      onClick={() => setActiveTab('admissionSteps')}
                      className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm text-left flex items-center gap-4 hover:shadow-md transition-all cursor-pointer"
                    >
                      <div className="h-12 w-12 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center shrink-0">
                      ...
                      <div>
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Admissions</p>
                        <h4 className="text-2xl font-black text-slate-800 leading-none mt-1">{admissionSteps.length}</h4>
                        <p className="text-[10px] text-slate-400 font-medium mt-1">Journey Steps</p>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Officials</p>
                        <h4 className="text-2xl font-black text-slate-800 leading-none mt-1">{officials.length}</h4>
                    ...
                    <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm text-left flex flex-col justify-between">
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-2.5">
                          ...
                          <button 
                            onClick={() => { setActiveTab('admissionSteps'); setShowAdmissionStepModal(true); }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 rounded-xl text-xs font-bold transition-all border border-yellow-200/20 text-left cursor-pointer"
                          >
                            <FileText className="h-4.5 w-4.5" /> Add Admission Step
                          </button>
                          <button 
                            onClick={() => { setActiveTab('officials'); setShowOfficialModal(true); }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 bg-purple-50 text-[#652d90] hover:bg-purple-100 rounded-xl text-xs font-bold transition-all border border-purple-200/20 text-left cursor-pointer"
                          >
                            <Users className="h-4.5 w-4.5" /> Add School Official
                          </button>
                        </div>
                      </div>
            ...
            {activeTab === 'admissionSteps' && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white border border-slate-200/80 rounded-3xl p-6 gap-4 shadow-sm text-left">
                  <div>
                    <h2 className="text-xl font-black text-slate-800 font-serif">Admission Journey Steps</h2>
                    <p className="text-slate-400 text-xs font-light">Create and edit the step-by-step application, examination, and enrollment journey</p>
                  </div>
                  <button 
                    onClick={() => {
                      setEditingAdmissionStepId(null);
                      setNewAdmissionStep({
                        num: String(admissionSteps.length + 1).padStart(2, '0'),
                        title: '',
                        desc: '',
                        icon: 'ClipboardList',
                        detailsText: '',
                ...
                {admissionSteps.length === 0 ? (
                  <div className="bg-white border border-slate-200/80 rounded-3xl p-16 text-center shadow-sm">
                    ...
                    <h3 className="text-slate-700 font-bold text-sm">No steps defined</h3>
                    <p className="text-slate-400 text-xs font-light mt-1 max-w-sm mx-auto">Create steps (e.g. Registration, Assessment, Interview) to display on the Admissions page.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                    {admissionSteps.map((step) => (
                      <div 
                        ...
                        className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col justify-between hover:shadow-md transition-all duration-300 relative group"
                      >
                        <div className="absolute top-0 left-0 w-full h-1.5 rounded-t-3xl bg-gradient-to-r from-[#652d90] to-yellow-500" />
                        
                        <div className="flex-1 flex flex-col gap-4 mt-2">
                          <div className="flex items-center gap-3">
                            <span className="w-10 h-10 bg-purple-50 text-[#652d90] font-black text-sm flex items-center justify-center rounded-xl shrink-0 border border-purple-100">
                              {step.num}
                            </span>
                            <div className="truncate">
                              <h4 className="font-extrabold text-slate-800 text-sm sm:text-base truncate">{step.title}</h4>
                              <span className="text-[10px] text-slate-400 font-mono block">Icon: {step.icon} • Order: {step.order}</span>
                            </div>
                          </div>

                          <p className="text-slate-500 font-light text-xs sm:text-sm leading-relaxed line-clamp-3">
                            {step.desc}
                          </p>

                          <div className="flex flex-col gap-2 pt-2 border-t border-slate-100">
                            <strong className="text-[11px] text-slate-700 font-bold uppercase tracking-wider block">Requirements ({step.details?.length || 0})</strong>
                            <div className="flex flex-col gap-1.5">
                              {(step.details || []).map((detail, i) => (
                                <div key={i} className="flex items-start gap-2 text-slate-500 text-xs">
                                  <div className="w-1.5 h-1.5 rounded-full bg-[#652d90] mt-1.5 shrink-0" />
                        ...
                        <div className="flex gap-2 justify-end mt-5 pt-3 border-t border-slate-100">
                          <button
                            onClick={() => handleEditAdmissionStep(step)}
                            className="px-2.5 py-1.5 bg-purple-50 hover:bg-purple-100 text-[#652d90] rounded-lg text-[11px] font-bold transition-all border border-purple-200/20 cursor-pointer flex items-center gap-1"
                          >
                            <Edit className="h-3.5 w-3.5" /> Edit
                          </button>
            ...
            {activeTab === 'officials' && (
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white border border-slate-200/80 rounded-3xl p-6 gap-4 shadow-sm text-left">
                  <div>
                    <h2 className="text-xl font-black text-slate-800 font-serif">School Team & Officials</h2>
                    <p className="text-slate-400 text-xs font-light">Manage academic leaders, professional faculty, and dedicated administrators</p>
                  </div>
                  <button 
                    onClick={() => {
                      setEditingOfficialId(null);
                    ...
                    className="px-5 py-2.5 bg-[#652d90] hover:bg-[#4b1f6b] text-white rounded-xl text-xs font-bold transition-all shadow-md active:scale-95 flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
                  >
                    <Plus className="h-4.5 w-4.5" /> Add Official
                  </button>
                </div>
                ...
                {officials.length === 0 ? (
                  <div className="bg-white border border-slate-200/80 rounded-3xl p-16 text-center shadow-sm">
                    ...
                    <h3 className="text-slate-700 font-bold text-sm">No officials registered</h3>
                    <p className="text-slate-400 text-xs font-light mt-1 max-w-sm mx-auto">Create team members to display on the Team page.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 text-left">
                    {officials.map((official) => (
                      <div 
                        key={official.id} 
                        className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm flex flex-col justify-between hover:shadow-md transition-all duration-300 relative group animate-fadeIn"
                      >
                        <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 shrink-0">
                          <ImageWithFallback 
                            ...
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out"
                          />
                          <span className="absolute top-3 left-3 bg-[#652d90]/90 backdrop-blur-sm text-white font-extrabold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full border border-purple-100/20">
                        ...
                        <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                          <div className="flex flex-col gap-1">
                            <h3 className="text-base font-black text-slate-800 font-serif leading-tight">{official.name}</h3>
                            <span className="text-xs font-bold text-[#652d90] tracking-wide">{official.position}</span>
                            
                            <div className="flex flex-col gap-0.5 mt-2 text-slate-500 text-[11px]">
                              <span className="truncate"><strong>Degree:</strong> {official.qualification}</span>
                              <span><strong>Exp:</strong> {official.experience}</span>
                              <span className="truncate"><strong>Email:</strong> {official.email}</span>
                              <span><strong>Order:</strong> {official.order}</span>
                            </div>
                          </div>
      ...
      <AnimatePresence>
        ...
        {showAdmissionStepModal && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-5">
                <h3 className="text-lg font-black text-slate-800 font-serif">{editingAdmissionStepId ? 'Edit Admission Journey Step' : 'Add Admission Journey Step'}</h3>
                <button 
                  onClick={() => {
                    setShowAdmissionStepModal(false);
                    setEditingAdmissionStepId(null);
              ...
              <form onSubmit={handleCreateAdmissionStepSubmit} className="flex flex-col gap-4">
                
                {/* Step Number */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Step Number Badge</label>
                  <input 
                    type="text" 
                    placeholder="e.g. 01, 02, 03..." 
                    value={newAdmissionStep.num}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, num: e.target.value})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                {/* Title */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Step Title</label>
                  <input 
                    type="text" 
                    placeholder="e.g. Online Registration" 
                    value={newAdmissionStep.title}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, title: e.target.value})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                {/* Description */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Description Summary</label>
                  <textarea 
                    rows={3} 
                    placeholder="Fill and submit the online application form to register your child's inquiry..." 
                    value={newAdmissionStep.desc}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, desc: e.target.value})}
                    required
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light resize-none focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                {/* Icon Selection */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Journey Icon</label>
                  <select 
                    value={newAdmissionStep.icon}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, icon: e.target.value})}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  >
                    <option value="ClipboardList">ClipboardList Icon</option>
                    <option value="FileCheck">FileCheck Icon</option>
                    <option value="FileSpreadsheet">FileSpreadsheet Icon</option>
                    <option value="Compass">Compass Icon</option>
                  </select>
                </div>

                {/* Requirements / Checklist (One per line) */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Required Guidelines & Checklists (one per line)</label>
                  <textarea 
                    rows={4} 
                    placeholder="e.g. Photocopy of birth certificate.&#10;Three passport size photographs." 
                    value={newAdmissionStep.detailsText}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, detailsText: e.target.value})}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light resize-none focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input 
                    type="number" 
                    placeholder="e.g. 1" 
                    value={newAdmissionStep.order}
                    onChange={e => setNewAdmissionStep({...newAdmissionStep, order: parseInt(e.target.value) || 0})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                <div className="flex gap-3 justify-end mt-2">
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      editingAdmissionStepId ? 'Save Changes' : 'Add Step'
                    )}
                  </button>
                </div>
        ...
        {showOfficialModal && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-5">
                <h3 className="text-lg font-black text-slate-800 font-serif">{editingOfficialId ? 'Edit Official Member' : 'Add Official Member'}</h3>
                <button 
                  onClick={() => {
                    setShowOfficialModal(false);
                    setEditingOfficialId(null);
                    setNewOfficial({ name: '', position: '', category: 'teachers', image: '', email: '', qualification: '', experience: '', order: 0 });
                  }} 
                  className="p-1 hover:bg-slate-100 rounded-lg cursor-pointer"
                >
                  <X className="h-5 w-5 text-slate-400" />
              ...
              <form onSubmit={handleCreateOfficialSubmit} className="flex flex-col gap-4">
                
                {/* Full Name */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Full Name</label>
                  <input 
                    type="text" 
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Position / Title</label>
                  <input 
                    type="text" 
                ...
                {/* Category Selection */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Department Category</label>
                  <select 
                    value={newOfficial.category}
                    onChange={e => setNewOfficial({...newOfficial, category: e.target.value as any})}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  >
                    <option value="leadership">Leadership (Administration Heads)</option>
                    <option value="teachers">Teaching Faculty (Academic Teachers)</option>
                    <option value="admin">Administration (Support Staff, Accounts)</option>
                  </select>
                </div>

                {/* Image URL */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Photo URL</label>
                  <input 
                    type="url" 
                    placeholder="https://images.unsplash.com/... or custom URL" 
                    value={newOfficial.image}
                ...
                {/* Email Address */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Professional Email Address</label>
                  <input 
                    type="email" 
                    placeholder="e.g. name@dhungesanghu.edu.np" 
                ...
                {/* Qualification */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Highest Qualification</label>
                  <input 
                    type="text" 
                    placeholder="e.g. M.Sc. in Physics" 
                    value={newOfficial.qualification}
                    onChange={e => setNewOfficial({...newOfficial, qualification: e.target.value})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                {/* Experience */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-0.5">Years of Experience</label>
                  <input 
                    type="text" 
                    placeholder="e.g. 8+ Years Experience" 
                    value={newOfficial.experience}
                    onChange={e => setNewOfficial({...newOfficial, experience: e.target.value})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input 
                    type="number" 
                    placeholder="e.g. 1" 
                    value={newOfficial.order}
                    onChange={e => setNewOfficial({...newOfficial, order: parseInt(e.target.value) || 0})}
                    required
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#652d90] rounded-xl text-xs sm:text-sm font-light focus:outline-none transition-all duration-300"
                  />
                </div>
